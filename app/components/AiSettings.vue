...
const form = reactive({ provider: settings.value.provider, model: settings.value.model, apiKey: settings.value.apiKey })
...
function onSave() {
  save({ provider: form.provider, model: form.model.trim(), apiKey: form.apiKey.trim() })
  open.value = false
}
function onClear() {
  clearKey()
    ...
    <template #body>
      <div class="space-y-4">
        ...
        <p class="rounded-lg bg-elevated/60 px-3 py-2 text-xs text-muted">
          🔒 Your key never leaves your browser except to call your chosen provider through this site. It is not stored on our servers. Leave it blank to use the site's shared (rate-limited) model.
        </p>
      </div>
    </template>
