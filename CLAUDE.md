...
- **Features**:
  ...
  - `pages/admin/index.vue` — moderation queue + overview stats (admin-gated in UI), and links to the two authoring tools below. `server/api/admin/moderate.post.ts` — approve/reject, **service-role**, guarded by `server/utils/auth.ts requireAdmin()`.
  - **Blog** (`posts` table): `pages/admin/posts.vue` manages them; public at `pages/blog/index.vue` + `pages/blog/[slug].vue`. Posts are either original or **curated** (`is_external`) — curated posts *require* `author_name` + `source_url` (enforced in `server/utils/posts.ts`, by a CHECK constraint, and in the UI), render a visible credit line, and set `rel="canonical"` to the original so we don't compete with the source in search. Default `excerpt_only` is true: host a summary, link out for the full article.
  - **Content builder** (`sections` + `lessons` tables): `pages/admin/content.vue` creates/reorders/publishes curriculum at runtime; public at `pages/learn/index.vue` + `pages/learn/[section]/[lesson].vue`. This exists because Vercel's filesystem is read-only at runtime and Nuxt Content compiles at build time — so admin-authored content *cannot* be markdown files. Lesson fields mirror the markdown frontmatter contract (`access`, `video` id/start/end, `quiz`, `passScore`). `server/utils/content-builder.ts` normalises input and accepts a YouTube URL *or* bare id for the video.
    - Quiz **answers never reach the client** — `server/api/learn/[section]/[lesson].get.ts` strips them, and grading goes through the *same* `/api/quiz` engine as markdown lessons. `server/utils/quiz-source.ts` `loadQuizPool()` resolves a `/learn/<section>/<lesson>` path against the `lessons` table and anything else against the `docs` collection, so both lesson kinds share one grader. Builder lessons use `quizId = learn:<lesson.id>` (keyed on row id, so renaming a slug doesn't orphan attempt history).
- **Database**: `supabase/migrations/` — idempotent; all tables + RLS + `is_admin()` + auto-create-profile trigger + role-escalation guard. **Apply it manually** in the Supabase SQL Editor. Tables: `profiles`, `lesson_progress`, `quiz_attempts`, `resources`, `projects`, `comments`, `posts` (blog), `sections` + `lessons` (content builder). `comments/resources/projects.user_id` FK → `profiles(id)` (enables author embedding).
