...
1. **Data Sync / Connect** stages source data as small datasets (with incremental refresh and filters for Salesforce).
2. **Dataflow (legacy) or Recipes (modern)** transform and combine that staged data.
3. The result is **optimized datasets** — high-volume, fast-query columnar storage.
4. **Monitoring** keeps the whole pipeline healthy.
